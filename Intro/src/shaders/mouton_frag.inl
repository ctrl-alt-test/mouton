...
const char *mouton_frag =
 ...
 "uniform float iTime;"
 "const float y=3.141592653589,m=0.,s=1.,x=2.,f=3.,a=4.,z=5.,l=6.,c=7.,p=8.,e=9.,r=10.,d=11.,n=12.;"
 ...
 "{"
   ...
   "float i=max(max(f.x,f.y),f.z),e=min(min(d.x,d.y),d.z);"
   "if(i>e||e<0.)"
     "return vec2(-1.);"
   "return vec2(i,e);"
 "}"
 ...
 "{"
   ...
   "f+=smoothstep(2.,0.,length(m-z))*.02;"
   "f*=smoothstep(.5,0.,sunDir.y);"
   "vec2 d=m*200.,s=fract(d)-.5,a=floor(d);"
   "vec3 p=t(vec3(abs(a),abs(a.x)));"
   "float l=p.z*.1;"
   "f+=smoothstep(l,l*.01,length(p.xy-.5+s))*(cos(iTime*3.*p.y+p.z*3.14)*.5+.5)*2.;"
 ...
 "{"
   ...
   "float i=m.x+317.*m.y+157.*m.z,x=w(i),f=w(i+1.),z=w(i+317.),a=w(i+318.),p=w(i+157.),l=w(i+158.),e=w(i+474.),k=w(i+475.),d=x,c=f-x,n=z-x,r=p-x,u=x-f-z+a,b=x-z-p+e,T=x-f-p+l,h=-x+f+z-a+p-l-e+k;"
   "return-1.+2.*(d+c*y.x+n*y.y+r*y.z+u*y.x*y.y+b*y.y*y.z+T*y.z*y.x+h*y.x*y.y*y.z);"
 "}"
 "vec2 u(vec3 v)"
 "{"
   "v.xz-=anvilPos.xz;"
 ...
 "{"
   ...
   "float e=m.x<0.&&d.y<0.?-1.:1.;"
   "return e*sqrt(min(dot(d,d),dot(m,m)));"
 "}"
 ...
 "{"
   ...
   "if(x<5.)"
     "{"
       ...
       "vec3 f=i;"
       "f.xz=t(f.xz,y*.2);"
       "float d=u(f-vec3(.5,.2+sin(f.x*2.)*.15,0.),vec3(2.,.1+sin(f.z*50.)*.02,.75)*.25);"
       "if(d<s.x)"
         "s=vec2(d,e);"
       "float z=max(length(v.xz+vec2(cos(v.y*.25+iTime)*v.y*.2,-.075))-smoothstep(3.1,0.,v.y)*.1-.02,v.y-3.1);"
       "if(z<s.x)"
         "s=vec2(z,r);"
       "return s;"
     "}"
 ...
 "{"
   ...
   "if(d<3.)"
     "{"
       "float e=pow(i((v-m-.1)*2.)*.5+.5,.75)*2.-1.;"
       "d=d+.05-e*.2;"
           ...
           "else"
             ...
             "else"
               ...
               "else"
                 ...
                 "else"
                   " if(b.y==r)"
                     "q=vec3(0.,.1,0.),A*=vec3(1.),U*=vec3(1.)*w;"
                   "else"
                     " if(b.y==e)"
                       "{"
                         "vec3 E=y-flowerPos;"
 ...
 "{"
   ...
   "{"
     ...
     "E.y-=1.4;"
     "E=g(iTime*5.)*E;"
     "r=mix(r,mix(vec3(1.,.8,.1),vec3(1.),smoothstep(-.1,.5,f.y)),smoothstep(0.,-.01,D(E,1.7,.5))*excited);"
   "}"
   "r*=smoothstep(0.,10.,iTime);"
   "r=mix(r,vec3(1.),smoothstep(163.,165.,iTime));"
   "r/=1.+pow(length(f),3.)*.1;"
