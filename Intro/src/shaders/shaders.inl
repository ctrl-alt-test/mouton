...
const char *mouton_frag =
 ...
 "{"
   ...
   "return y<.4?"
     "(y-=pow((C(v*.9)*.5+C(v*1.6)*.3+C(v*2.7)*.1)*.5+.5,3.)*.45*(1.-exp((-t+135.)*3.)),vec2(y,12)):"
     "vec2(9e7,0);"
 "}"
 ...
 "{"
   ...
   "float W=length(gl_FragCoord.xy/v.xx-vec2(.5,.3)),_=max(.137,smoothstep(147.,146.,t));"
   "X*=smoothstep(_,_-.005,W);"
   "float Y=fract(23.*pow(W,.25));"
   "Y-=smoothstep(.95,.99,Y);"
   "X=mix(X,mix(vec3(1,.6,0),vec3(1,0,0),pow(Y,1.)),smoothstep(.135,.136,W)*smoothstep(147.,148.,t));"
...
const char *mouton_vert =
 ...
 "{"
   ...
   "else if(v<96.)"
     "{"
       "float f=v-88.,X=smoothstep(6.,9.,f);"
       "y=vec3(0,0,f*.25-11.);"
       "i=vec3(-2,0,-8);"
       "d=vec3(-.5,.25-X,1);"
       "a.y=.5-X;"
