...
const char *fxaa_frag =
 ...
 "{"
   ...
   "vec4 i=vec4(y,y-v*.5);"
   "vec3 m=textureLod(P,i.zw,0.).xyz,c=textureLod(P,i.zw+vec2(1,0)*v.xy,0.).xyz,F=textureLod(P,i.zw+vec2(0,1)*v.xy,0.).xyz,x=textureLod(P,i.zw+vec2(1)*v.xy,0.).xyz,d=textureLod(P,i.xy,0.).xyz,s=vec3(.299,.587,.114);"
   "float a=dot(m,s),n=dot(c,s),t=dot(F,s),z=dot(x,s),b=dot(d,s),H=min(b,min(min(a,n),min(t,z))),r=max(b,max(max(a,n),max(t,z)));"
   "vec2 K=vec2(-(a+n-t-z),a+t-n-z);"
   "float l=1./(min(abs(K.x),abs(K.y))+1./128.);"
   "K=min(vec2(8),max(vec2(-8),K*l))*v.xy;"
   "vec3 p=.5*(textureLod(P,i.xy+K*(1./3.-.5),0.).xyz+textureLod(P,i.xy+K*(2./3.-.5),0.).xyz),G=p*.5+.25*(textureLod(P,i.xy+K*-.5,0.).xyz+textureLod(P,i.xy+K*.5,0.).xyz);"
   "float C=dot(G,s);"
   "f=C<H||C>r?"
     "vec4(p,1):"
     "vec4(G,1);"
