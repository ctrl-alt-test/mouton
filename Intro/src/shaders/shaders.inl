...
const char *mouton_frag =
 ...
 "{"
   ...
   "float c=clamp(dot(v,x),0.,1.);"
   "return B(c,30./(1.+y))*2.+B(c,2e4/(1.+y));"
 "}"
 ...
 "{"
   ...
   "vec3 l=B(vec3(abs(d),abs(d.x)));"
   "float n=l.z*.06;"
   "f+=vec3(1,.9,.1)*s*smoothstep(.5,-1.,e.y);"
   "f+=smoothstep(n,n*.01,length(l.xy-.5+a))*(1.-m);"
 ...
 "{"
   ...
   "if(f<3.)"
     "{"
       ...
       "f=f+.05-a*.2;"
       "float n=mod(t*s.x,2.),z=smoothstep(0.,.5,n),d=smoothstep(.5,1.,n),e=smoothstep(1.,1.5,n),r=smoothstep(1.5,2.,n);"
       "vec4 k=vec4(d*(1.-d),r*(1.-r),z*(1.-z),e*(1.-e)),o=n*.5-vec4(d,r,z,e);"
       "o*=m.x;"
       "vec3 I=v;"
       "I.x-=.8;"
       "I.z-=2.+o.x;"
 ...
 "{"
   "vec3 m=abs(v);"
   "return max(m.z-i.y,D(D(v.x*.9+v.y*.5,-v.x*.9+v.y*.5,y),-v.y,y)-i.x*.5);"
 "}"
 "vec2 H(vec3 v)"
 "{"
   "v-=i;"
 ...
 "{"
   ...
   "for(int m=1;m<=2;m++)"
     "{"
       "float c=float(m)/2*i,n=I(v+c*y).x;"
       "f+=(c-n)*s;"
       "s*=.95;"
     "}"
 ...
 "{"
   "vec3 s=1./f;"
   "float c=9e7,m;"
   "vec2 d=g(v-y-vec3(0,3,-2),f,vec3(3,3,7),s);"
   "if(d.y>0.)"
     "{"
       "m=max(d.x,0.);"
       "for(int n=0;n<128;n++)"
       ...
       "if(m<d.y)"
         "c=m;"
     "}"
   "d=g(v-z-vec3(0,5,-5),f,vec3(1.5,5,1),s);"
   "if(d.y>0.)"
     "{"
       "m=max(d.x,0.);"
       "for(int n=0;n<128;n++)"
       ...
       "if(m<d.y)"
         "c=min(c,m);"
     "}"
   "d=g(v-i-vec3(0,5,-5),f,vec3(1.5,5,1),s);"
   "if(d.y>0.)"
     "{"
       "m=max(d.x,0.);"
       "for(int n=0;n<128;n++)"
       ...
       "if(m<d.y)"
         "c=min(c,m);"
     "}"
   "d=g(v-x-vec3(0,5,0),f,vec3(2,5,1),s);"
   "if(d.y>0.)"
     "{"
       "m=max(d.x,0.);"
       "for(int n=0;n<128;n++)"
       ...
       "if(m<d.y)"
         "c=min(c,m);"
     "}"
   "d=g(v-r-vec3(0,3,2),f,vec3(4,4,5),s);"
   "if(d.y>0.)"
     "{"
       "m=max(d.x,0.);"
       "for(int n=0;n<128;n++)"
     ...
     "}"
   "m=-(v.y+.4)*s.y;"
   "if(m>0.&&length((v+f*m).xz-r.xz)<10.)"
     "{"
       "for(int n=0;n<128;n++)"
         "{"
           "float a=A(v+f*m).x;"
 ...
 "{"
   ...
   "return clamp(f,0.,1.);"
 "}"
 "float E(vec2 v,float i,float y,float x)"
 "{"
   "v.x=abs(v.x);"
   "float s=(i-y)/x,c=sqrt(1.-s*s),m=dot(v,vec2(-s,c));"
   "return m<0.?"
     "length(v)-i:"
     "m>c*x?"
       "length(v-vec2(0,x))-y:"
 ...
 "{"
   ...
   "vec3 K=(b*(G+r*.5+o*2.+H*2.)+w+F*P+N)*s;"
   "float Q=length(y-v);"
   "K=mix(K,A(m,c,s),smoothstep(90.,1e2,Q));"
   "K=clamp(K,0.,1.);"
   "if(a.y==0.)"
     "{"
       "float O=length(c)*.5,U=cos(atan(c.x,c.y)*15.-t*3.-O*30.*k.y);"
 ...
 "{"
   "vec2 i=vec2(.81,-.59),c=vec2(-i.x,i.y);"
   "v.x=abs(v.x);"
   "v-=2.*max(dot(i,v),0.)*i;"
   "v-=2.*max(dot(c,v),0.)*c;"
   "v.x=abs(v.x);"
   "v.y-=m;"
   "vec2 x=y*vec2(-i.y,i.x)-vec2(0,1);"
 ...
 "{"
   ...
   "float d=F(i,s);"
   "vec3 b=i+s*d,z=J(b),H=A(i,s,b,z,y);"
   "vec2 l=vec2(abs(y.x*5.-.3)-1.5,y.y*5.-1.4);"
   "l=C(t*5.)*l;"
   "float r=1.4+.2*sin(t*20.);"
   "r*=smoothstep(.5,1.,k.x);"
   "float Q=F(l,r,.5);"
   "vec3 O=mix(vec3(1,.6,0),vec3(1,.2,0),smoothstep(-.1,.6,F(l,r*.5,.5)))*1.3;"
   "H=mix(H,O,smoothstep(0.,-.01,Q)*k.x);"
   "H=pow(H,vec3(1./2.2));"
   "H=pow(H,vec3(1,1.05,1.1));"
   "H*=smoothstep(0.,10.,t);"
   "float p=length(gl_FragCoord.xy/v.xx-vec2(.5,.3)),K=max(.137,smoothstep(157.,156.,t));"
   "H*=smoothstep(K,K-.005,p);"
   "float I=p,e=smoothstep(.135,.136,I)*smoothstep(157.,158.,t);"
   "I=fract(23.*pow(I,.25));"
   "I-=smoothstep(.95,.99,I);"
   "vec3 P=mix(vec3(1,.6,0),vec3(1,0,0),pow(I,1.));"
   "H=mix(H,P,e);"
...
const char *fxaa_frag =
 ...
 "{"
   ...
   "vec4 i=vec4(y,y-v*.5);"
   "vec3 m=textureLod(O,i.zw,0.).xyz,c=textureLod(O,i.zw+vec2(1,0)*v.xy,0.).xyz,F=textureLod(O,i.zw+vec2(0,1)*v.xy,0.).xyz,x=textureLod(O,i.zw+vec2(1)*v.xy,0.).xyz,d=textureLod(O,i.xy,0.).xyz,s=vec3(.299,.587,.114);"
   "float a=dot(m,s),n=dot(c,s),H=dot(F,s),z=dot(x,s),K=dot(d,s),b=min(K,min(min(a,n),min(H,z))),e=max(K,max(max(a,n),max(H,z)));"
   "vec2 t=vec2(-(a+n-H-z),a+H-n-z);"
   "float l=1./(min(abs(t.x),abs(t.y))+1./128.);"
   "t=min(vec2(8),max(vec2(-8),t*l))*v.xy;"
   "vec3 p=.5*(textureLod(O,i.xy+t*(1./3.-.5),0.).xyz+textureLod(O,i.xy+t*(2./3.-.5),0.).xyz),G=p*.5+.25*(textureLod(O,i.xy+t*-.5,0.).xyz+textureLod(O,i.xy+t*.5,0.).xyz);"
   "float C=dot(G,s);"
   "f=C<b||C>e?"
     "vec4(p,1):"
     "vec4(G,1);"
