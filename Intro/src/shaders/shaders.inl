 ...
 "{"
   ...
   "else if(r.y==3.)"
     "{"
       ...
       "float am=abs(sin(q*5.))-smoothstep(.15,.7,j)*4.;"
       "am=smoothstep(.449,.45,am);"
       "vec3 ac=mix(vec3(0),vec3(.75,.5,1)*.5,am);"
       "ac=mix(vec3(.7,.7,0),ac,smoothstep(.06,.1,j));"
       "ac*=smoothstep(135.2,135.6,t);"
       "S+=ac;"
       "L*=0.;"
     "}"
   "else if(r.y==5.)"
   ...
   "X=mix(X,A(l,m,b),smoothstep(90.,1e2,O));"
   "X=clamp(X,0.,1.);"
   "if(r.y==0.)"
     "{"
       "float j=length(m)*.5;"
       "X=mix(X,mix(mix(vec3(1,.5,0),vec3(.8,.5,1),(cos(j*5.+t*5.)*.5+.5)*k.y),vec3(1),smoothstep(-j,j,cos(atan(m.x,m.y)*15.-t*3.-j*30.*k.y))),k.x);"
     "}"
...
const char *fxaa_frag =
 ...
 "{"
   ...
   "float a=dot(textureLod(L,m.zw,0.).xyz,x),s=dot(textureLod(L,m.zw+vec2(1,0)*v.xy,0.).xyz,x),c=dot(textureLod(L,m.zw+vec2(0,1)*v.xy,0.).xyz,x),d=dot(textureLod(L,m.zw+vec2(1)*v.xy,0.).xyz,x),t=dot(textureLod(L,m.xy,0.).xyz,x);"
   "vec2 i=vec2(-a-s+c+d,a+c-s-d);"
   ...
   "float l=dot(r,x);"
   "f=l<min(t,min(min(a,s),min(c,d)))||l>max(t,max(max(a,s),max(c,d)))?"
     "vec4(X,1):"
     "vec4(r,1);"
 "}";
