...
const char *mouton_frag =
 ...
 "{"
   "return clamp(1.-i*1.5*(.5*y-J(v+x*y*.5).x+.95*(y-J(v+x*y).x)),0.,1.);"
 "}"
...
const char *fxaa_frag =
 ...
 "{"
   ...
   "float m=dot(textureLod(X,s.zw,0.).xyz,x),c=dot(textureLod(X,s.zw+vec2(1,0)*v.xy,0.).xyz,x),d=dot(textureLod(X,s.zw+vec2(0,1)*v.xy,0.).xyz,x),e=dot(textureLod(X,s.zw+vec2(1)*v.xy,0.).xyz,x),L=dot(textureLod(X,s.xy,0.).xyz,x);"
   ...
   "float l=dot(o,x);"
   "f=l<min(L,min(min(m,c),min(d,e)))||l>max(L,max(max(m,c),max(d,e)))?"
     "vec4(a,1):"
     "vec4(o,1);"
 "}";
