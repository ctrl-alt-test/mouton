...
const char *mouton_frag =
 ...
 "{"
   ...
   "return y<.4?"
     "(y-=pow((C(v*.9)*.5+C(v*1.6)*.3+C(v*2.7)*.1)*.5+.5,3.)*.45*(1.-exp((-t+137.3)*3.)),vec2(y,12)):"
     "vec2(9e7,0);"
 "}"
 ...
 "{"
   ...
   "vec2 c=A(t*2e-4)*v*2e2,m=floor(c);"
   "vec3 d=h(vec3(abs(m),abs(m.x)));"
   "float f=d.z*.06;"
   "return s*smoothstep(.13,.2701,length(v-x-vec2(.05))+.004*C(1e2*vec3(v-x,0)))*vec3(1,.9,.1)*smoothstep(.5,-1.,r.y)+smoothstep(f,f*.01,length(fract(c)-.5+(d.xy-.5)))*(1.-s)+exp(-length(v-x)*2.)*.1+pow(y,2.);"
 "}"
 ...
 "{"
   ...
   "if(c<3.)"
     "{"
       "float d=pow(C((v-i+vec3(.05,0,.5))*2.)*.5+.5,.75)*2.-1.;"
       "c=c+.05-d*.2;"
 ...
 "{"
   ...
   "for(int c=1;c<3;c++)"
     "{"
       "float m=float(c)*.5*s,d=I(v+m*x).x;"
       "i+=(m-d)*f;"
       "f*=.95;"
     "}"
 ...
 "{"
   ...
   "return i<0.?"
     ...
     "i>c*s?"
       ...
       "dot(v,vec2(c,m))-y;"
 "}"
 "float E(vec2 v,float s,float x)"
 "{"
