...
const char *mouton_frag =
 ...
 "{"
   ...
   "float t=max(max(m.x,m.y),m.z),n=min(min(d.x,d.y),d.z);"
   "return t>n||n<0.?"
     "vec2(-1):"
     "vec2(t,n);"
 "}"
 ...
 "{"
   "float y=max(dot(v,normalize(i+v)),0.),d=2e3/x,c=3./x;"
   "return(pow(y,d)*(d+2.)+pow(y,c)*(c+2.)*2.)*.04;"
 "}"
 "vec3 h(vec3 v)"
 "{"
 ...
 "{"
   ...
   "vec2 m=A(t*2e-4)*v*2e2,d=floor(m);"
   "vec3 s=h(vec3(abs(d),abs(d.x)));"
   "float c=s.z*.06;"
 ...
 "{"
   "vec2 m=vec2(length(v.xz),v.y),d=vec2(y,x),c=vec2(y-i,2.*x),s=vec2(m.x-min(m.x,m.y<0.?"
     "i:"
     "y),abs(m.y)-x),t=m-d+c*clamp(dot(d-m,c)/dot(c,c),0.,1.);"
   "return(t.x<0.&&s.y<0.?"
     "-1.:"
 ...
 "{"
   ...
   "if(y<5.)"
     "{"
       ...
       "vec3 f=m;"
       "const float c=g*.2;"
       "float d=atan(f.z,f.x),s=length(f.xz);"
       "d=mod(d-c/2.,c)-c/2.;"
       "f.xz=vec2(cos(d),sin(d))*s;"
 ...
 "{"
   ...
   "if(c<3.)"
     "{"
       ...
       "c=c+.05-z*.2;"
       "float d=mod(t*s.x,2.),n=smoothstep(0.,.5,d),e=smoothstep(.5,1.,d),l=smoothstep(1.,1.5,d),r=smoothstep(1.5,2.,d);"
 ...
 "{"
   ...
   "for(int d=1;d<3;d++)"
     "{"
       "float c=float(d)*.5*i,s=I(v+c*y).x;"
       "m+=(c-s)*f;"
       "f*=.95;"
     "}"
 ...
 "{"
   "vec3 s=1./m;"
   "float c=9e7,f;"
   "vec2 d=h(v-y-vec3(0,3,-2),m,vec3(3,3,7),s);"
   "if(d.y>0.)"
     "{"
       "f=max(d.x,0.);"
       "for(int e=0;e<128;e++)"
       ...
       "if(f<d.y)"
         "c=f;"
     "}"
   "d=h(v-i-vec3(0,5,-5),m,vec3(1.5,5,1),s);"
   "if(d.y>0.)"
     "{"
       "f=max(d.x,0.);"
       "for(int e=0;e<128;e++)"
       ...
       "if(f<d.y)"
         "c=min(c,f);"
     "}"
   "d=h(v-z-vec3(0,5,-5),m,vec3(1.5,5,1),s);"
   "if(d.y>0.)"
     "{"
       "f=max(d.x,0.);"
       "for(int e=0;e<128;e++)"
       ...
       "if(f<d.y)"
         "c=min(c,f);"
     "}"
   "d=h(v-x-vec3(0,5,0),m,vec3(2,5,1),s);"
   "if(d.y>0.)"
     "{"
       "f=max(d.x,0.);"
       "for(int e=0;e<128;e++)"
       ...
       "if(f<d.y)"
         "c=min(c,f);"
     "}"
   "d=h(v-l-vec3(0,3,2),m,vec3(4,4,5),s);"
   "if(d.y>0.)"
     "{"
       "f=max(d.x,0.);"
       "for(int e=0;e<128;e++)"
 ...
 "{"
   ...
   "y.x*=v.x/v.y;"
   "vec3 s=c,a=B(s,e)*normalize(vec3(y,n-length(y)*o));"
   "float l=E(s,a);"
   "vec3 O=s+a*l;"
   ...
   "else if(H.y==3.)"
     "{"
       ...
       "{"
         "vec3 am=reflect(a,w),av=normalize(vec3(1,1.5,-1)),ac=vec3(-av.x,av.y*.5,av.z);"
         "float ad=0.;"
         "ad+=h(am,av,.1);"
         "ad+=h(am,ac,2.)*.1;"
         "ad+=h(am,normalize(av+vec3(.2,0,0)),.3);"
         "ad+=h(am,normalize(av+vec3(.2,0,.2)),.5);"
         "ad+=h(am,normalize(ac+vec3(.1,0,.2)),8.)*.5;"
         "ad/=5.;"
         "P=(mix(mix(vec3(.3,.3,0),vec3(.1),smoothstep(-.7,.2,am.y)),vec3(.3,.65,1),smoothstep(0.,1.,am.y))+ad*vec3(1,.9,.8))*mix(.15,.2,ax)*sqrt(g)*2.5;"
       "}"
       "I(O);"
   ...
   "vec3 X=clamp(mix((S*(Q+N*.5+J*2.+K*2.)+P+L*U+R)*b,A(a,y,b),smoothstep(90.,1e2,l)),0.,1.);"
   "if(H.y==0.)"
     "{"
...
const char *mouton_vert =
 "#version 150\n"
 "in vec4 O;"
 "out vec3 m,s,y,x,i,z,l,r,c,e,d;"
 "out vec2 a,k;"
 "out float b,n,p,o,w;"
 "uniform float t;"
 "void main()"
 "{"
   ...
   "p=0.;"
   "if(v<10.)"
     "m=vec3(1,.2,1),s=vec3(1,1,.5),d=vec3(0,.45,1),a.y=.25,n=4.,c=vec3(0,2.5,-3.7),e=vec3(0,3,0);"
   "else if(v<20.)"
     "m=vec3(1,.2,.25),s=vec3(1,1,2),d=vec3(0,.5,1),a.y=.25,n=4.,c=vec3(5,.5,0),e=vec3(0,.5,1);"
   "else if(v<30.)"
     "{"
       "float f=v-20.;"
       "m=vec3(1,.2,.25);"
...
const char *fxaa_frag =
 ...
 "{"
   ...
   "float c=dot(textureLod(X,m.zw,0.).xyz,x),s=dot(textureLod(X,m.zw+vec2(1,0)*v.xy,0.).xyz,x),d=dot(textureLod(X,m.zw+vec2(0,1)*v.xy,0.).xyz,x),z=dot(textureLod(X,m.zw+vec2(1)*v.xy,0.).xyz,x),t=dot(textureLod(X,m.xy,0.).xyz,x);"
   "vec2 i=vec2(-c-s+d+z,c+d-s-z);"
   "float e=1./(min(abs(i.x),abs(i.y))+1./128.);"
   "i=min(vec2(8),max(vec2(-8),i*e))*v.xy;"
   "vec3 a=.5*(textureLod(X,m.xy+i*(1./3.-.5),0.).xyz+textureLod(X,m.xy+i*(2./3.-.5),0.).xyz),n=a*.5+.25*(textureLod(X,m.xy+i*-.5,0.).xyz+textureLod(X,m.xy+i*.5,0.).xyz);"
   "float l=dot(n,x);"
   "f=l<min(t,min(min(c,s),min(d,z)))||l>max(t,max(max(c,s),max(d,z)))?"
     "vec4(a,1):"
     "vec4(n,1);"
 "}";
