...
const char *mouton_frag =
 ...
 "{"
   ...
   "for(int d=1;d<3;d++)"
     "{"
       "float c=float(d)*.5*m,s=I(v+c*y).x;"
       "i+=(c-s)*f;"
       "f*=.95;"
     "}"
   ...
   "else if(H.y==3.)"
     "{"
       ...
       "{"
         ...
         "float ad=0.;"
         "ad+=h(am,av,0.);"
         "ad+=h(am,normalize(av+vec3(.2,0,0)),2.);"
         "ad+=h(am,normalize(av+vec3(.2,0,.2)),4.);"
         "ad+=h(am,ac,20.)*.1;"
         "ad+=h(am,normalize(ac+vec3(.1,0,.2)),80.)*.5;"
         "ad/=5.;"
...
const char *fxaa_frag =
 ...
 "{"
   ...
   "float c=dot(textureLod(X,m.zw,0.).xyz,x),s=dot(textureLod(X,m.zw+vec2(1,0)*v.xy,0.).xyz,x),d=dot(textureLod(X,m.zw+vec2(0,1)*v.xy,0.).xyz,x),t=dot(textureLod(X,m.zw+vec2(1)*v.xy,0.).xyz,x),n=dot(textureLod(X,m.xy,0.).xyz,x);"
   "vec2 i=vec2(-c-s+d+t,c+d-s-t);"
   "float z=1./(min(abs(i.x),abs(i.y))+1./128.);"
   "i=min(vec2(8),max(vec2(-8),i*z))*v.xy;"
   "vec3 a=.5*(textureLod(X,m.xy+i*(1./3.-.5),0.).xyz+textureLod(X,m.xy+i*(2./3.-.5),0.).xyz),e=a*.5+.25*(textureLod(X,m.xy+i*-.5,0.).xyz+textureLod(X,m.xy+i*.5,0.).xyz);"
   "float l=dot(e,x);"
   "f=l<min(n,min(min(c,s),min(d,t)))||l>max(n,max(max(c,s),max(d,t)))?"
     "vec4(a,1):"
     "vec4(e,1);"
 "}";
